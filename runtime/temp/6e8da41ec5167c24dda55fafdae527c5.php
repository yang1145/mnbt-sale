<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:59:"D:\wwwroot\zj.cc\public/../app/admin\view\default\pays.html";i:1713890984;s:51:"D:\wwwroot\zj.cc\app\admin\view\default\header.html";i:1738745069;s:51:"D:\wwwroot\zj.cc\app\admin\view\default\footer.html";i:1688394438;}*/ ?>
<!DOCTYPE html>
<html lang="zh">
<head>
...
<title><?php echo $webname; ?> - 支付</title>
...
<br/>
<button onclick='add()' class="btn btn-primary me-1">
<span class="mdi mdi-plus" aria-hidden="true"></span>添加支付通道
</button>
...
<td>
<a href="/admin/pays/<?php echo $p['id']; ?>" class="btn btn-success btn-sm me-1"><i class="mdi mdi-pencil"></i>配置参数</a>
...
<div style="text-align:center"><?php echo $pays->render(); ?></div>
...
layer.open({
        ...
        shadeClose: true,
        title:"添加支付通道",
        content: '<div style="width:350px;" class="card-body"><div class="tab-content"><div class="mb-3"><label for="web_site_title" class="form-label">名称</label><input class="form-control" type="text" name="name" value="" placeholder=""></div><div class="mb-3"><label for="type" class="form-label">支付插件</label><div class="form-controls"><select name="plugins" class="form-select" id="type"><option value="">不选择</option><?php if(is_array($payss) || $payss instanceof \think\Collection || $payss instanceof \think\Paginator): $i = 0; $__LIST__ = $payss;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><option value="<?php echo $p; ?>"><?php echo $p; ?></option><?php endforeach; endif; else: echo "" ;endif; ?> </select></div><br/><button type="button" onclick="add1()" class="btn btn-primary me-1">添加</button></div> </div> </div>',
  });
...
function add1(){
        ...
        var plugins = $("select[name='plugins']").val();
        $.ajax({
            type:'POST',
            url:'',
            data:{
...
var id = id.join(",");
if(id==""){
layer.alert("未选择支付通道!",{icon:2,shade:0.8});
}else{
        layer.confirm('确定要删除ID为<span style="color:#0b5ed7">'+ id +'</span>的支付通道吗？', function (){
            del1(id)
        });
    }
}
...
function qbdel(){
        layer.confirm('确定要删除<span style="color:#0b5ed7">全部</span>的支付通道吗？', function (){
            qbdel1()
        });
    }
