<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:60:"D:\wwwroot\zj.cc\public/../app/admin\view\default\order.html";i:1713529570;s:51:"D:\wwwroot\zj.cc\app\admin\view\default\header.html";i:1738745069;s:51:"D:\wwwroot\zj.cc\app\admin\view\default\footer.html";i:1688394438;}*/ ?>
 <!DOCTYPE html>
<html lang="zh">
<head>
...
<title><?php echo $webname; ?> - 订单管理</title>
...
<td>
                          <div class="form-check">
                            <input type="checkbox" class="form-check-input ids" name="ids" value="<?php echo $o['id']; ?>">
                            <label class="form-check-label" for="ids-1"></label>
                          </div>
                        </td>
                <td scope="row"><?php echo $o['id']; ?></td>
                <td><?php echo $o['cartid']; ?></td>
                <td><?php echo $o['userid']; ?></td>
                <td><?php echo $o['user']; ?></td>
                <td><?php echo date("Y-m-d H:i:s",$o['atime']); ?></td>
                <td><?php echo date("Y-m-d H:i:s",$o['ztime']); ?></td>
                <td>
<?php if($o['state']=="1"): ?>
...
<td>
<a href="/admin/order/<?php echo $o['id']; ?>" class="btn btn-success btn-sm me-1"><i class="mdi mdi-pencil"></i>查看信息</a>
...
var id = id.join(",");
if(id==""){
layer.alert("未选择订单!",{icon:2,shade:0.8});
}else{
        layer.confirm('确定要删除ID为<span style="color:#0b5ed7">'+ id +'</span>的订单吗？', function (){
            del1(id)
        });
    }
}
...
function qbdel(){
        layer.confirm('确定要删除<span style="color:#0b5ed7">全部</span>的订单吗？', function (){
            qbdel1()
        });
    }
